...
- [ ] **Step 1: Measure both HEVC cases before writing anything**

Two separate measurements, and the step needs both. The divisor decides what a
rate-stating stream reports; the ceiling decides whether a stream stating nothing
is still rejected, and its effective value halves when the divisor goes from two
to one. The ceiling is measured for H.264 at `1200000/1` and is unmeasured for
HEVC.

First the divisor, on a stream that does state a rate:

```bash
ffmpeg -v error -y -i tests/assets/hevc.mp4 -c copy -f hevc /tmp/rate_stated.hevc
ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate -of csv=p=0 /tmp/rate_stated.hevc
```

Record the value in the code comment beside the divisor. If the reported tick rate
is not 25 for this asset, stop and report: the divisor is then not one and the
spec's premise needs revisiting.

Then the ceiling, on a stream that states none. Produce a raw HEVC stream whose
bitstream carries no timing and read its `r_frame_rate` the same way. Record what
the demuxer actually reports there, and confirm the ceiling still rejects it at a
divisor of one. If it does not, the ceiling needs a codec-specific value and the
refusal in a later task would otherwise swallow every rate-less HEVC source
silently rather than by the rule that is supposed to catch it.
...
In `tests/probe/test_raw_stream.py`, add:
...
The stream must carry no video usability information. Verified that no stream copy
produces one: `-x264opts no-timing-info` is rejected outright as a bad option, and
routing a committed asset through the metadata bitstream filter with the tick rate
zeroed leaves the timing intact -- the result still reports a real rate and decodes
its full frame count. So an encoder invocation is the only route, and the asset is
committed rather than generated at test time.

Produce it once, by hand, outside the test suite. A subprocess call to `ffmpeg`
carries no linking consequence, which is what makes committed H.264 media the
sanctioned route here; the encoder guard scans string literals in `.py` files
under `src/` and `tests/` only, so neither this document nor
`tests/assets/README.md` is in its scope. Write a short clip with the encoder's
video usability information disabled, and place the result at
`tests/assets/raw_no_declared_rate.h264`.

Verify **both** properties before committing it. Reporting an implausible tick
rate is not sufficient on its own -- a corrupt stream reports the same value, and
a check on the rate alone accepts an undecodable file:

```bash
ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate \
    -of csv=p=0 tests/assets/raw_no_declared_rate.h264
ffprobe -v error -select_streams v:0 -count_frames \
    -show_entries stream=nb_read_frames -of csv=p=0 \
    tests/assets/raw_no_declared_rate.h264
```

Expected: a rate the plausibility ceiling rejects, such as `1200000/1`, **and** a
frame count equal to the number of frames encoded. A stream reporting `N/A` for
the count, or logging a reference to a non-existent parameter set, is corrupt
rather than timing-free and is the wrong asset.

Add a row for it to `tests/assets/README.md` carrying the exact generation
command, as every other committed asset there does, and saying that it cannot be
derived by copying.

- [ ] **Step 2: Register the asset name**

`tests/assets/` files are reached through the `asset` helper rather than by a
relative path, because the committed originals are never handed out directly.
That helper's `name` parameter is a closed `Literal`; add
`"raw_no_declared_rate.h264"` to it. Without the entry the call does not
type-check, and a bare `Path("tests/assets/...")` resolves against the process
...
def test_the_committed_rate_less_stream_is_refused(tmp_path: Path) -> None:
    path = asset("raw_no_declared_rate.h264", tmp_path / "rate_less.h264")
