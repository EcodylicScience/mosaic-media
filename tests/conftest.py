...
from tests.helpers.corpus import generate_vfr_video, generate_video
...
@pytest.fixture(scope="session")
def corpus_vfr(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Path]:
    # 30 fps with a 10 fps stretch over frames 100..149: the variable-rate
    # shape of a recorder dropping frames while the machine is busy.
    root = tmp_path_factory.mktemp("corpus_vfr")
    yield generate_vfr_video(
        root / "vfr.mp4",
        frames=300,
        fps=30.0,
        slow_fps=10.0,
