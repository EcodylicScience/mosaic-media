...
# The one-way layering among the stdlib-only layers, keyed by the first path
# component under src/mosaic_media (module stem for top-level files). Each
# entry lists the layers a file there may reach with a relative import. The
# heavier layers (io, transcode, cli) are absent on purpose: reaching them
# from here is a violation, and a new layer must be added explicitly.
...
def test_core_relative_imports_follow_the_layering() -> None:
    """Relative imports between the stdlib-only layers stay one-way.

    The absolute-import test above cannot see these edges (relative imports
    ...
    importing thumbnail would otherwise pass both guards.
    """
    modules = source_files()
    assert modules, "stdlib-only core modules not found"
    offenders: dict[str, set[str]] = {}
